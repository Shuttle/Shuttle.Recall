	...
	public class DefaultTransactionScope : ITransactionScope
	{
        ...
        public void Complete()
        {
			if (_ignore)
			{
                if (_log.IsVerboseEnabled)
                {
                    _log.Verbose(string.Format(RecallResources.VerboseTransactionScopeAmbientCompleted, _name,
                                              Thread.CurrentThread.ManagedThreadId));
                }
            ...
            if (_log.IsVerboseEnabled)
            {
                _log.Verbose(string.Format(RecallResources.VerboseTransactionScopeCompleted, _name,
                                          Thread.CurrentThread.ManagedThreadId));
            }
        }
    }
